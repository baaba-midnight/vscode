...
suite('ChatAgents', function () {
	...
	const studentPolicyService: IStudentPolicyService = {
	...
	setup(() => {
		...
		chatAgentService = store.add(new ChatAgentService(contextKeyService, new TestConfigurationService(), studentPolicyService));
	});
